...
// Placeholder for Lottie. Replace with your Lottie component (e.g. lottie-react)
const RoxAILottie = ({ isOpen }: { isOpen: boolean }) => (
  <div className="w-12 h-12 rounded-full bg-gradient-to-br from-cyan-400 to-emerald-300 flex items-center justify-center shadow-lg">
    <Bot className="w-7 h-7 text-gray-900" />
  </div>
);

export default function WakandaFooter() {
  ...
  const fadeRise = {
    ...
    visible: { opacity: 1, y: 0, transition: { type: "spring", stiffness: 120, damping: 18, delay: 0.2 } }
  };

  // Panel/modal variants
  const panelVariants = {
    hidden: { y: "100%", opacity: 0, filter: "blur(10px)" },
    visible: { y: 0, opacity: 1, filter: "blur(0px)", transition: { type: "spring", stiffness: 200, damping: 22 } },
    exit: { y: "100%", opacity: 0, filter: "blur(10px)", transition: { type: "spring", stiffness: 300, damping: 30 } }
  };
  const roxAIVariants = {
    hidden: { scale: 0.7, opacity: 0, y: 40 },
    visible: { scale: 1, opacity: 1, y: 0, transition: { type: "spring", stiffness: 300, damping: 22 } },
    exit: { scale: 0.7, opacity: 0, y: 40, transition: { type: "spring", stiffness: 400, damping: 30 } }
  };
  ...
  return (
    <>
      {/* Fixed Footer */}
      <motion.footer
        className="fixed bottom-0 left-0 right-0 w-full z-50 bg-gray-900 border-t border-gray-800/60 shadow-2xl"
        variants={fadeRise}
        initial="hidden"
        animate="visible"
      >
        ...
        {showRoxAI && (
          ...
          >
            <motion.div
              ...
              animate="visible"
              exit="exit"
              className="relative bg-gray-900 rounded-t-2xl md:rounded-2xl shadow-2xl p-6 w-full max-w-md border border-emerald-400/30"
              onClick={e => e.stopPropagation()}
            >
